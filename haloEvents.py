...
"api key created": 901,
    ...
    "auto. file integ. scan schedule modified": 104,
    ...
    "automatic file integ. scanning disabled": 106,
    "automatic file integ. scanning enabled": 108,
    ...
    "daemon self-verification scan requested": 938,
    "daemon self-verification scheduled modfied": 909,
    "daemon version changed": 934,
    ...
    "file integrity baseline invalid": 118,
    "file integrity baseline": 115,
    "file integrity change detected": 117,
    ...
    "file integrity scan failed": 116,
    "file integrity scan requested": 114,
    "ghostports expiration time modified": 301,
    "ghostports login failure": 305,
    "ghostports login success": 306,
    "ghostports provisioning": 311,
    "ghostports session close": 316,
    "halo api authentication success": 919,
    "halo authentication settings modified": 920,
    "halo backup codes generated": 921,
    ...
    "halo login failure": 922,
    "halo login success": 923,
    "halo logout": 924,
    "halo password changed": 925,
    "halo password recovery failed": 926,
    "halo password recovery request failed": 927,
    "halo password recovery requested": 928,
    "halo password recovery success": 929,
    "halo session timeout": 930,
    "halo user account locked": 931,
    "halo user account unlocked": 932,
    "halo user activation failed": 911,
    "halo user deactivated": 912,
    "halo user invited": 913,
    "halo user modified": 914,
    "halo user reinvited": 915,
    "halo user reactivated": 916,
    "halo user added": 941,
    "halo user deleted": 942,
    ...
    "local account deleted (linux only)": 810,
    "local account modification requested": 809,
     "local account ssh keys update requested": 811,
    "log-based intrusion detection disabled": 400,
    "log-based intrusion detection enabled": 401,
    "log-based intrusion detection policy assigned": 402,
    "log-based intrusion detection policy created": 403,
    "log-based intrusion detection policy deleted": 404,
    "log-based intrusion detection policy exported": 405,
    "log-based intrusion detection policy modified": 406,
    "log-based intrusion detection policy unassigned": 407,
    "log-based intrusion detection rule matched": 408,
    "master account linked": 917,
    "multiple accounts detected with same UID": 935,
    "multiple root accounts detected": 936,
    "multiple root accounts detected (linux only)": 937,
    ...
    "server IP address changed": 726,
    "server access scan requested": 812,
    "server deleted": 717,
    "server firewall modified": 700,
    ...
    "sms phone number verified": 940,
    "software vulnerability detected": 603,
    "software vulnerability exception created": 608,
    "software vulnerability exception deleted": 609,
    "software vulnerability exception expired": 610,
...
def encodeStringAsCEF(str):
    if type(str) is list:
        accum = ""
        for elem in str:
            accum += encodeStringAsCEF(elem) + " "
        return accum
    elif isinstance(str, types.StringTypes):
        str = str.replace("\\","\\\\")
        str = str.replace("=","\\=")
        str = str.replace("","")
    return str
...
def convertToCEF(ev):
    ...
    if (customCount > 6):
        ...
        if ('name' in ev):
            evname = ev['name']
        print >>sys.stderr, "Found %d custom fields, exceeded 6 allowed, event=%s" % (customCount, evname)
    return str
